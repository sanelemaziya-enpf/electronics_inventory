...
if (isset($_POST['change_password'])) {
    ...
    $stmt_pw->bind_param("i", $_SESSION['user_id']);
    $stmt_pw->execute();
    $user = $stmt_pw->get_result()->fetch_assoc();
    $stmt_pw->close();
    ...
    if (!$user || !password_verify($current_password, $user['password'])) {
        ...
        $stmt_upw = $conn->prepare("UPDATE users SET password=? WHERE id=?");
        $stmt_upw->bind_param("si", $hashed_password, $_SESSION['user_id']);
        $stmt_upw->execute();
        $stmt_upw->close();
...
// Get user statistics
$stmt_s1 = $conn->prepare("SELECT COUNT(*) as count FROM assets WHERE created_by = ?");
$stmt_s1->bind_param("i", $_SESSION['user_id']);
$stmt_s1->execute();
$assets_count = $stmt_s1->get_result()->fetch_assoc()['count'];
$stmt_s1->close();

$stmt_s2 = $conn->prepare("SELECT COUNT(*) as count FROM asset_assignments WHERE assigned_to = ? AND status='active'");
$stmt_s2->bind_param("i", $_SESSION['user_id']);
$stmt_s2->execute();
$assignments_count = $stmt_s2->get_result()->fetch_assoc()['count'];
$stmt_s2->close();

$user_stats = [
    'assets_created' => $assets_count,
