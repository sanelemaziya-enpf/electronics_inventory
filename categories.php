...
if (isset($_POST['add_category'])) {
    ...
    if ($stmt->execute()) {
        ...
        logActivity($_SESSION['user_id'], 'CREATE', 'categories', $conn->insert_id, "Added category: $name");
    } else {
...
if (isset($_POST['edit_category'])) {
    ...
    if ($stmt->execute()) {
        ...
        $message_type = "success";
        logActivity($_SESSION['user_id'], 'UPDATE', 'categories', $id, "Updated category: $name");
    } else {
...
if (isset($_GET['delete']) && $_SESSION['role'] === 'super_admin') {
    ...
    $check = $conn->query("SELECT COUNT(*) as count FROM assets WHERE category_id = $id");
    $count = $check->fetch_assoc()['count'];
    
    if ($count > 0) {
        $message = "Cannot delete category. It has $count associated assets.";
        $message_type = "danger";
    } else {
        $conn->query("DELETE FROM categories WHERE id = $id");
        $message = "Category deleted successfully!";
        $message_type = "success";
        logActivity($_SESSION['user_id'], 'DELETE', 'categories', $id, "Deleted category");
    }
}

// Get all categories with asset counts
$categories = $conn->query("SELECT c.*, COUNT(a.id) as asset_count, u.full_name as created_by_name
                           FROM categories c
        ...
        <div class="top-navbar">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <small class="text-muted">Organize your assets by categories</small>
                </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-4">
                    <div class="category-card">
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="badge bg-primary"><?php echo $category['asset_count']; ?> Assets</span>
                            <div>
                                <button class="btn btn-sm btn-outline-primary me-1" onclick="editCategory(<?php echo $category['id']; ?>, '<?php echo htmlspecialchars($category['name'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($category['description'], ENT_QUOTES); ?>')">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <?php if ($_SESSION['role'] === 'super_admin' && $category['asset_count'] == 0): ?>
                                <a href="?delete=<?php echo $category['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this category?')">
                        ...
                        <small class="text-muted d-block mt-3">
                            <i class="fas fa-user me-1"></i> By <?php echo htmlspecialchars($category['created_by_name']); ?>
                        </small>
                    </div>
                </div>
                <?php endwhile; ?>
